#Computes segregating sites, private sites and fixed opposite sites for each simulation

arp<-readLines("./multiple_cl_ne-temp/multiple_ne_cl-temp_1_1.arp") #genetic data simulated by fastsimcoal
...
pop7L <- grep("7_", arp)
pop7L<- sort(c(pop7L,pop7L+1))
pop8L <- grep("8_", arp)
pop8L<- sort(c(pop8L,pop8L+1))
...
{
  r<-unlist(strsplit(pop7[j], split="\t"))
  a<-unlist(strsplit(r[3], split=""))
  if (j==1) {mat7<-matrix(nrow=length(pop7),ncol=(length(a)-1))
  mat7[j,]<-a[2:length(a)]}
  
  else {mat7[j,]<-a[2:length(a)]}
}
...
for (i in 1:length(vet7))
	...
	stato7[i]<-as.numeric(unique(mat7[,i]))}
	else {stato7[i]=4}}


for (j in 1: length(pop8))
{
  r<-unlist(strsplit(pop8[j], split="\t"))
  a<-unlist(strsplit(r[3], split=""))
  if (j==1) {mat8<-matrix(nrow=length(pop8),ncol=(length(a)-1))
  mat8[j,]<-a[2:length(a)]}
  
  else {mat8[j,]<-a[2:length(a)]}
}
...
for (i in 1:length(vet8))
	...
	stato8[i]<-as.numeric(unique(mat8[,i]))}
	else {stato8[i]=4}}
...
fixed<-c(fixed,fix) #pairwise fixed sites
}
...
posi<-c()
for (i in 1: length(priv)){if (priv[i]==1) {posi<-c(posi,i)}} ###posi=columns of pri of which we have segregating sites
